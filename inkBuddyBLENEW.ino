...
#define ADC_PIN 35
#define CONV_FACTOR 1.82
// Pangodream_18650_CL BL;
Pangodream_18650_CL BL(ADC_PIN,CONV_FACTOR,20);

void Voltage(){
    String s = String(BL.getBatteryChargeLevel());
    ...
    // display.print((char*)BL.getBatteryChargeLevel());
...
class SecondCharacteristicCallbacks: public BLECharacteristicCallbacks{
  void onWrite(BLECharacteristic *pCharacteristic){
    ...
    // else if(pData[0]==0xFF){
      ...
      if(len==512){
      for(int i=0;i<512;i++){
        arrayOfBits[i+(512*counter1)] = pData[i];
      }      
      counter1++;      
      ...
      pCharacteristic->notify();
        ...
        display.setCursor(0,0);
        // display.drawBitmap(0,0,arrayOfBits,250,128,GxEPD_BLACK);
        display.drawBitmap(arrayOfBits,4000);
